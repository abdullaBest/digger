...
class Popup {
	...
	async input(header: string, text: string = ""): Promise<string> {
		this.label.innerHTML = header;
	...
	private async apply(resolver?: () => any): Promise<string|number|any> {
 ...
 * @param message
 */
function popupListSelect(
	message: string,
	propagete?: (el: HTMLElement) => void
): Promise<string> {
 ...
 * @param message
 */
function popupListSelectMultiple(
	message: string,
	propagate?: (el: HTMLElement) => void
