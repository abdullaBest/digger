...
import { MapTilesetSystem, MapSystem } from ".";
import { AssetContentTypeComponent, AssetContentTypeTileset } from "../app/assets";
...
class TilesetRender {
	...
	_isPosInClipbounds(pos_x: number, pos_y: number) {
		...
		return !(
			pos_x < this.min_x - threshold_w ||
			pos_x > this.max_x + threshold_w ||
			pos_y < this.min_y - threshold_h ||
			pos_y > this.max_y + threshold_h
		);
	}
...
class RenderTilesetSystem extends MapSystem {
	...
	constructor(scene_core: SceneCore) {
		...
		this.scene_core = scene_core;
		this.tileset_render = new TilesetRender(
			this.scene_core,
			this.scene_core.systems.tileset as MapTilesetSystem
		);
