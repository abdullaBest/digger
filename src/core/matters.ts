...
class Matters {
	...
	get(id?: string | null): Matter | null {
		if (!id) {
			return null;
		}
		// using by "pointer" id as "**someid"
		const m = id.startsWith("**") ? this.list[id.substring(2)] : this.list[id];
	...
	traverse(
		id: string,
		callback_values?:
			| ((matter: Matter, key: string, value: any) => void)
			| null,
		callback_links?: (matter: Matter) => void
	) {
	...
	create(
		...
		if (_inherites) {
			...
			if (!inherited) {
				throw new Error(
					`Matters::create error - Matter ${_id} tries to inherit ${_inherites} wich does not exist.`
				);
			}
	...
	remove(id: string) {
		...
		if (matter.dependents) {
			throw new Error(
				`Matters::remove error - Can't remove matter ${id} while it has ${matter.dependents} dependents.`
			);
		}
