			...
			if (b.elapsed < 1) {
				...
				for (let i = 0; i < body.contacts; i++) {
					...
					if (
						...
						c.normal_y < 0 &&
						c.time <= 0 &&
						!this.scene_collisions.bodies[c.id ?? ""]
					) {
				...
				if ((component as any).tileref && component.inherites) {
					component = this.scene_core.matters.get(
						component.inherites
	...
	touchFallingBlock(id: string, shaketime: number = 1) {
		const activate = (blockid: string, colliderid: string) => {
			...
			for (const k in this.scene_collisions.colliders) {
				...
				const collides = this.scene_collisions.thresholdAABBCollision(ca, cb, -0.01, 0);
		...
		for (const k in this.scene_collisions.colliders) {
			...
			// x threshold decreased - not gona affect corner touches
			const collides = this.scene_collisions.thresholdAABBCollision(
				ca,
				cb,
				-0.01,
				0.01
