...
import { PropertyInput, PropertyInputMutator } from "../document/controls_property_input";

type InspectorMattersOnchangeCallback = (
	matter: Matter,
	key: string,
	value_old: any,
	value_new: any
...
export default class InspectorMatters {
	...
	init(mutators?: { [id: string]: PropertyInputMutator }) {
		...
		const onchange = (
			...
			this.events?.dispatchEvent(
				new CustomEvent("change", {
					detail: { id: matter.id, key, value_old, value_new },
				})
			);
		};
		listenClick(
			...
			(ev) => {
				ev.stopPropagation();
				this.events?.dispatchEvent(
					new CustomEvent("clone", { detail: { id } })
				);
			},
			this._listeners
		);
		listenClick(
			...
			(ev) => {
				ev.stopPropagation();
				this.events?.dispatchEvent(
					new CustomEvent("delete", { detail: { id } })
				);
			},
			this._listeners
		);
	...
	init_input(
		...
		key: string,
		entry: HTMLElement,
		onchange?: InspectorMattersOnchangeCallback
	): HTMLElement {
		const input = new PropertyInput(this.mutators[key] ?? null);
		this.inputs[key] = input;
		const input_container = input.init(
			matter[key],
			(value) => {
			...
			addEventListener(
				{
					callback: () => {
						this.events?.dispatchEvent(
							new CustomEvent("external", {
								detail: { key, value: matter.get(key) },
							})
						);
					},
					name: "click",
					node: btn_external_ref,
				},
				this._listeners
			);
			addEventListener(
				{
					callback: () => {
						this.events?.dispatchEvent(
							new CustomEvent("plug", {
								detail: { key, value: matter.get(key), matter },
							})
						);
					},
					name: "click",
	...
	init_field(
		...
		const input_controls = this.init_field_controls(
			matter,
			key,
			entry,
			onchange
		...
		if (
			matter.is_link(key) &&
			this.matters.get(value)?.get("owner") == matter.id
		) {
			const subcontainer = this.init_container();
			subcontainer.id = `sub-${matter.id}-${key}`;
	...
	draw_field(
		...
		entry: HTMLElement = this.entries[key]
	) {
