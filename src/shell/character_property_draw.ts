...
export default class CharacterPropertyDraw {
	...
	drawConfig() {
		this.core.addWrite(
			"mass",
			() => this.character.body.mass,
			(v) => (this.character.body_mass = this.character.body.mass = v)
		);
		this.core.addWrite(
			"drag",
			() => this.character.body.drag,
			(v) => (this.character.body_drag = this.character.body.drag = v)
		);
		this.core.addWrite("movement_speed"); 
		this.core.addWrite("movement_acceleration"); // acceleration actually
		this.core.addWrite("jump_force"); // acceleration actually
