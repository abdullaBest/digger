...
app.post(
	"/assets/upload/thumbnail/:id",
	upload_thumbnail.single("files"),
	updateThumbnail
);
...
// files access
app.get("/assets/list", setNoCache, async (req, res) => {
	res.json(assets.keys());
});

app.get("/assets/get/:id", setNoCache, async (req, res) => {
	const id = req.params.id;
	const asset = assets.get(id);
	if (!asset) {
		res.statusCode = 500;
	...
	const info = Object.assign({ url: `/assets/load/${id}/${revision}` }, asset);
...
app.get("/assets/load/:id/:revision", async (req, res, next) => {
	...
	const filename =
		asset.revision_files["r" + revision] ||
		asset.revision_files["r" + asset.revision];
	sendFile(filename, asset.type, res).catch((err) => {
		if (err.status !== 404) return next(err); // non-404 error
		// file for download not found
		res.statusCode = 404;
